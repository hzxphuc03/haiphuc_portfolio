...
export const HeroSection = () => {
  ...
  const containerOpacity = useTransform(scrollYProgress, [0, 0.6, 1], [1, 1, 0]);
  const headingY = useTransform(scrollYProgress, [0, 1], [0, -100]);
  const headingOpacity = useTransform(scrollYProgress, [0, 0.4, 0.7], [1, 1, 0]);
  const headingBlur = useTransform(scrollYProgress, [0, 0.5], [0, 8]);
  const cardsY = useTransform(scrollYProgress, [0, 1], [0, 80]);
  const cardsOpacity = useTransform(scrollYProgress, [0, 0.3, 0.6], [1, 1, 0]);
  ...
  return (
    <motion.section
      ...
      className="relative min-h-screen py-20 md:py-0 md:h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-[#0C0C0C]"
    >
      {/* Background Layers */}
      <div className="noise-bg" />
      <div className="crt-overlay" />
      
      {/* Ambient Glow Lights - parallax scale on scroll */}
      <motion.div
        style={{ scale: bgScale }}
      ...
      <div className="relative z-10 w-full max-w-7xl px-6 md:px-10 flex flex-col items-center">
        ...
        {/* Heading - parallax up + blur out on scroll */}
        <motion.div
          style={{ y: headingY, opacity: headingOpacity, filter: headingBlur }}
          className="mb-4"
        >
          <h1 className="hero-heading font-black uppercase text-[12vw] sm:text-[10vw] md:text-[8vw] lg:text-[7vw] leading-none tracking-tighter text-center">
            PHAM HAI PHUC
          </h1>
        </motion.div>
        ...
        <motion.div
          ...
          className="h-8 flex items-center justify-center mb-16"
        >
        ...
        {/* Centerpiece Bento Grid - fade & slide up on scroll */}
        <motion.div
          style={{ y: cardsY, opacity: cardsOpacity }}
          className="grid grid-cols-1 md:grid-cols-12 gap-4 w-full max-w-5xl h-auto md:h-[400px]"
        >
          {/* Card 1: Tech Stack Dock */}
          <ParallaxLayer speed={-0.2} className="md:col-span-4">
            <div className="bg-[#141414] border border-[#D7E2EA]/5 rounded-2xl p-6 flex flex-col justify-between overflow-hidden group hover:border-[#D7E2EA]/20 transition-colors min-h-[180px] md:min-h-0">
          ...
          <ParallaxLayer speed={0.1} className="md:col-span-5 relative">
            <div className="bg-[#141414] border border-[#D7E2EA]/5 rounded-2xl overflow-hidden group min-h-[300px] md:min-h-0 h-full">
          ...
          <ParallaxLayer speed={0.3} className="md:col-span-3 flex flex-col gap-4">
        ...
        >
          <div className="group relative px-10 py-4 cursor-pointer">
            <div className="group relative px-10 py-4 bg-transparent transition-all duration-500">
